...
- the 2026-05-26 sanitized private pilot feedback produced rule-quality fixes for Supabase context detection, API ownership false positives, secret placeholder false positives, and public provider debug/probe endpoints; it does not count as external public-beta design-partner evidence
- the 2026-05-26 sanitized private pilot recheck confirmed Supabase RLS and GitHub Actions noise were gone, then drove `StackInventory`, Supabase rule gating in `scan`, and route classification for admin-only, public content, internal/proxy, and scoped-token routes; it still does not count as external public-beta design-partner evidence

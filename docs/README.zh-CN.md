...
CLI 已发布到 npm：`ai-saas-guard@0.43.1`。GitHub Action 支持 `v0` 浮动标签，也支持固定版本标签，例如 `v0.43.1`。

| 模块 | 状态 |
| --- | --- |
| 公开 GitHub 仓库 | 已可用 |
| npm CLI | `ai-saas-guard@0.43.1` |
| GitHub Action | `zr9959/ai-saas-guard@v0` 或固定标签 `v0.43.1` |
| 输出格式 | 上线决策队列、短 summary、Terminal、JSON、SARIF 和 PR markdown |
| 项目配置 | `.ai-saas-guard.json` 支持规则开关、severity 覆盖、suppressions 和 fail threshold |
| 隐私模型 | 本地优先、只读扫描、不调用 LLM、不上传代码 |
| 当前版本 | `0.43.1` 加固 hosted rate-limit 失败处理、GitHub API URL 校验、Check Run 复现命令、MCP policy 模板和 Action summary 输出，同时继续保持 billing disabled |
| Action 标签 | `v0.43.1`、`v0` |
